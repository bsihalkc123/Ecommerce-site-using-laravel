          ...
          <div class="product-single__addtolinks">
            @if(Cart::instance('wishlist')->content()->where('id',$product->id)->count() > 0)
                <form method="POST" action="{{route('wishlist.item.remove',['rowId'=>Cart::instance('wishlist')->content()->where('id',$product->id)->first()->rowId])}}" id="frm-remove-item">
                  @csrf
                  @method('DELETE')
            <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart" onclick="getElementById('frm-remove-item').submit();"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
              ...
              <form method="POST" action="{{route('wishlist.add')}}" id="whishlist-form">
                ...
                <input type="hidden" name="price" value="{{ $product->sale_price== '' ? $product->regular_price : $product->sale_price }}"/>
              <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist" onclick="document.getElementById('whishlist-form').submit()"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
    ...
    <section class="products-carousel container">
      ...
      <div id="related_products" class="position-relative">
        <div class="swiper-container js-swiper-slider" data-settings='{
          ...
          @php
            $rGallery = array_filter(explode(',', $rproduct->images ?? ''));
            $rMainImage = $rproduct->image ?? ($rGallery[0] ?? 'no-image.png');
          @endphp
            ...
            <div class="pc__img-wrapper">
              ...
              {{-- MAIN IMAGE --}}
              <img src="{{ asset('uploads/products/' . $rMainImage) }}"
                    class="pc__img"
                    alt="{{ $rproduct->name }}">

              {{-- HOVER IMAGE (only one) --}}
              @if(isset($rGallery[1]))
                <img src="{{ asset('uploads/products/' . $rGallery[1]) }}"
